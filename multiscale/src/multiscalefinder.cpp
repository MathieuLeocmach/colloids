...
namespace Colloids {
    ...
    void MultiscaleFinder3D::set_deconv(bool value)
    {
    	if(this->octaves.size()>1)
			dynamic_cast<OctaveFinder3D*>(this->octaves[1])->set_deconv(value);
    }
    void MultiscaleFinder3D::load_deconv_kernel(const std::vector<PixelType> &kernel)
    {
    	if(this->octaves.size()>1)
			dynamic_cast<OctaveFinder3D*>(this->octaves[1])->load_deconv_kernel(kernel);
    }
