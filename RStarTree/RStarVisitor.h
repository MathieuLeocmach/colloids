...
struct RStarAcceptEnclosing
{
	...
	bool operator()(const Leaf * const leaf) const
	{
		return leaf && m_bound.encloses(leaf->bound);
	}

	private: RStarAcceptEnclosing(){}
};

// returns true if the specified bound is within the compared boundary
template <typename Node, typename Leaf>
struct RStarAcceptEnclosed
{
	const typename Node::BoundingBox &m_bound;
	explicit RStarAcceptEnclosed(const typename Node::BoundingBox &bound) : m_bound(bound) {}
	...
	bool operator()(const Leaf * const leaf) const
	{
		return leaf->bound.encloses(m_bound);
	}

	private: RStarAcceptEnclosed(){}
};
